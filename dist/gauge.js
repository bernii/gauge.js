...
(function() {
  var AnimatedText, AnimatedTextFactory, Bar, BaseDonut, BaseDouble, BaseGauge, Donut, Double, Gauge, GaugePointer, TextRenderer, ValueUpdater, addCommas, cutHex, formatNumber, mergeObjects, secondsToString, updateObjectValues,
  ...
  BaseDouble = (function(_super) {
    __extends(BaseDouble, _super);

    BaseDouble.prototype.lineWidth = 30;

    BaseDouble.prototype.displayedAngle = 0;

    BaseDouble.prototype.displayedValue = 0;

    BaseDouble.prototype.value = 33;

    BaseDouble.prototype.maxValue = 80;

    BaseDouble.prototype.minValue = -80;

    BaseDouble.prototype.paddingBottom = 0.1;

    BaseDouble.prototype.options = {
      lineWidth: 0.40,
      colorStart: "#d73d32",
      colorStop: "#53a93f",
      strokeColor: "#eeeeee",
    ...
    function BaseDouble(canvas) {
      this.canvas = canvas;
      BaseDouble.__super__.constructor.call(this);
      if (typeof G_vmlCanvasManager !== 'undefined') {
        this.canvas = window.G_vmlCanvasManager.initElement(this.canvas);
      }
      this.ctx = this.canvas.getContext('2d');
    ...
    BaseDouble.prototype.getAngle = function(value) {
      if (value < 0) {
        return (1.5 - value / this.minValue * (0.5 - this.options.angle)) * Math.PI;
      } else if (value > 0) {
        return (1.5 + value / this.maxValue * (0.5 - this.options.angle)) * Math.PI;
      } else {
        return 1.5 * Math.PI;
      }
    };

    BaseDouble.prototype.setOptions = function(options) {
      if (options == null) {
        options = null;
      }
      BaseDouble.__super__.setOptions.call(this, options);
      this.lineWidth = this.canvas.height * (1 - this.paddingBottom) * this.options.lineWidth;
      this.radius = this.canvas.height * (1 - this.paddingBottom) - this.lineWidth;
      this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
      this.render();
    ...
    BaseDouble.prototype.set = function(value) {
      this.value = value;
    ...
    BaseDouble.prototype.render = function() {
      var counterclockwise, displayedAngle, fillStyle, h, w;
      w = this.canvas.width / 2;
      h = this.canvas.height * (1 - this.paddingBottom);
      if (this.textField) {
        this.textField.render(this);
      ...
      if (displayedAngle < 1.5 * Math.PI) {
        ...
        fillStyle = this.options.colorStop;
      }
      this.ctx.strokeStyle = fillStyle;
      this.ctx.beginPath();
      this.ctx.arc(w, h, this.radius, 1.5 * Math.PI, displayedAngle, counterclockwise);
      this.ctx.stroke();
      return this.gp.update(true);
    };
  ...
  Double = (function(_super) {
    ...
    Double.prototype.strokeGradient = function(w, h, start, stop) {
      var grd;
      grd = this.ctx.createRadialGradient(w, h, start, w, h, stop);
      grd.addColorStop(0, this.options.shadowColor);
      grd.addColorStop(0.15, this.options._orgStrokeColor);
      grd.addColorStop(0.85, this.options._orgStrokeColor);
      grd.addColorStop(1, this.options.shadowColor);
      return grd;
    };

    Double.prototype.setOptions = function(options) {
      var h, start, stop, w;
      if (options == null) {
        options = null;
      }
      Double.__super__.setOptions.call(this, options);
      w = this.canvas.width / 2;
      h = this.canvas.height * (1 - this.paddingBottom);
